...
* **Replication Scope**: Implementation and evaluation of Persistence Landscape $L^1$ and $L^2$ norms for detecting phase transitions and market crashes in financial time series. Includes synthetic validation on 4D white noise variance scaling, Gamma transition state shifts, as well as real-world market crash analysis on US equity indices with Early Warning Signal (EWS) indicators. We take a detour by comparing Persistence Landscapes against Persistence Images for market crash detection. The full reproduction notebook can be found at `reproductions/gidea_katz_2018/reproduction_landscapes_of_crashes.ipynb`.
